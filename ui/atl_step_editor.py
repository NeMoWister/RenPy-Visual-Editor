...
"""
Пошаговый редактор ATL - альтернатива вводу сырого текста: пользователь
добавляет "шаги" (как в Ren'Py ActionEditor), в каждом шаге тумблерами
включает нужные свойства (xalign/yalign/zoom/rotate/alpha) и задаёт им
целевые значения + длительность/тип интерполяции - редактор сам собирает
из этого ATL-текст (linear/ease + pause, см. core/atl.py).

Это НЕ замена текстовому редактору, а надстройка над ним - шаги всегда
конвертируются в обычный ATL-текст, так что то, что не выражается шагами
(смена картинки, произвольные block:/pause-паттерны, pos/anchor и т.п.),
по-прежнему можно дописать руками во вкладке "Текст".
"""
...
class ToggleSwitch(QAbstractButton):
    """Тумблер-переключатель (iOS-style) - используется вместо обычных
    чекбоксов для включения/выключения свойства шага: компактнее и нагляднее
    читается рядом с числовым полем значения."""
...
class ValueSlider(QWidget):
    """Ползунок-полоска в том же стиле, что и таймлайн шагов (оранжевый
    трек + перетаскиваемый маркер) - используется ВМЕСТО обычного числового
    поля для значения свойства: тащим мышью или кликаем по треку, текущее
    число печатается прямо на полоске. Двойной клик позволяет ввести точное
    число с клавиатуры (для тонкой настройки)."""
...
def steps_from_atl_text(text: str, base_xalign: float, base_yalign: float,
    ...
    """Лучшее-старание разбор существующего ATL-текста в список шагов.
    Возвращает (steps, repeat, repeat_count, lossy) - lossy=True, если в
    тексте были конструкции, которые нельзя показать шагами (pos/anchor,
    смена картинки, block:, произвольный raw) - они молча отбрасываются
    из шагового вида (сам текст при этом не трогается, пока пользователь
    не переключится обратно на шаги и не сохранит)."""
...
class _StepTimeline(QWidget):
    ...
    клик выбирает шаг, перетаскивание маркера (кроме первого, t=0) меняет
    длительность ЭТОГО шага (время подхода к нему)."""
...
class AtlStepsPanel(QWidget):
    ...
    выбранного шага + кнопки добавить/удалить шаг + тумблер repeat.
    Испускает changed каждый раз, когда итоговый ATL-текст мог измениться."""
    ...
    def set_base(self, base_xalign: float, base_yalign: float, base_zoom: float):
        ...
        панели - используется при первом открытии, пока пользователь ещё
        не вводил свой ATL."""
    ...
    def load_from_text(self, text: str, base_xalign: float, base_yalign: float,
        ...
        """Пробует переразобрать шаги из (возможно, вручную отредактированного)
        текста ATL. Возвращает True, если разбор был "с потерями" (что-то не
        влезло в шаговую модель и было отброшено только из ЭТОГО представления,
        не из самого текста)."""
