...
def _node_header(node: SceneNode) -> str:
    """Верхняя подпись карточки ноды: тип + что она вызывает/на что ссылается."""
...
def _node_image_var(node: SceneNode) -> Optional[str]:
    """Если нода показывает картинку - имя ресурса, которое надо превьюшить."""
...
def _node_body_text(node: SceneNode) -> str:
    """Текст для нижнего поля превью, когда картинки нет."""
...
class NodeBoxItem(QGraphicsObject):
    ...
    def ports(self) -> List[QPointF]:
        """Точки выхода стрелок (в локальных координатах ноды), с которых можно
        тянуть новую связь мышью: у JUMP - одна, у MENU - по числу веток."""
    ...
    def play_click_pulse(self):
        """Короткая вспышка акцентным цветом вокруг ноды при выборе клика -
        небольшая, но заметная обратная связь на переключение между нодами."""
...
class ConnectionArrowItem(QGraphicsPathItem):
    """Пунктирная стрелка jump/menu -> label: кликабельна (можно выделить)
    и удаляема по Del - в отличие от обычных стрелок последовательности,
    это настоящая, разрываемая связь между нодами."""
...
class MiniMapView(QWidget):
    """Кликабельная мини-карта. В отличие от прежней версии, НЕ делит сцену
    с основным canvas и не рендерит настоящие (сложные) ноды через
    fitInView - вместо этого рисует простые прямоугольники по позициям нод
    с гарантированным минимальным размером. Это и дешевле (не перерисовывает
    сотни детальных QGraphicsItem на каждый refresh), и остаётся читаемым
    независимо от того, насколько большая сцена (раньше при большом графе
    fitInView зумил so far out, что миникарта становилась фактически
    невидимой)."""
...
class NodePreviewOverlay(QLabel):
    """Крупный предпросмотр текущей ноды - картинка (bg/sprite/cg) или текст
    реплики/нарратива. Живёт в правой панели над миникартой/редактором."""
...
class GraphNodeEditor(QWidget):
    """Компактная, полностью самостоятельная форма редактирования ноды для
    графового режима. Специально НЕ переиспользует основной NodeEditor и
    никогда никуда не репарентится между окнами - это отдельный, простой
    виджет без собственных тредов/таймеров/спеллчекера, живущий постоянно
    внутри NodeGraphCanvas. Изменения полей идут наружу через field_changed
    (row, {field: value}), сохранение делает владелец (panel)."""
    ...
    def flush(self):
        """Принудительно фиксирует ещё не улетевшие через debounce правки -
        иначе быстрое переключение на другую ноду (быстрее 400мс) тихо
        теряло последние введённые символы."""
        self._debounce.stop()
        if self._pending:
            self._emit_pending()
    ...
    def _build_wide_field(self, attr: str, kind: str, current_value, node: SceneNode):
        """Полноширинные виджеты редактора: карусели ресурсов (те же классы,
        что и в главном окне), пикер 'скрыть весь спрайт персонажа', и блок
        выбора аудио (комбобокс + ▶️/⏹️), тоже приведённый к главному окну."""
    ...
    def _build_carousel(self, attr: str, kind: str, current_value: str):
        """Тот же самый виджет-карусель, что и в главном редакторе
        (ui/resource_carousel.py) - просто размещён в другой компоновке,
        поэтому выбор фона/спрайта/CG работает идентично основному окну.
        Без tags_store карусель молча прячет выбор группы - поэтому он
        обязательно прокидывается сюда."""
        thumb = 150
    ...
    def _build_audio_row(self, attr: str, category: str, current_var: str):
        """Комбобокс с файлами категории + ▶️/⏹️ через общий audio-плеер
        приложения (тот же синглтон, что и в главном окне). Без визуальной
        волны/перетаскивания фейдов основного редактора - только выбор
        файла и прослушивание."""
        container = QWidget()
    ...
    def _build_character_combo(self, current_var: str) -> QComboBox:
        """Тот же принцип, что и char_combo в главном редакторе: первый
        пункт - рассказчик (без персонажа), дальше - персонажи проекта."""
        w = QComboBox()
    ...
    def _build_transition_combo(self, current_value: str) -> QComboBox:
        """Тот же список переходов (TRANSITIONS), что и в главном окне,
        редактируемый - чтобы нестандартные значения из импорта не терялись."""
        w = QComboBox()
    ...
    def _build_validated_text(self, attr: str, value) -> QWidget:
        """Многострочное поле реплики/нарратива с подсказкой о длине текста
        (зелёный/жёлтый/красный), как в главном редакторе - без спеллчекера
        (тот использует отдельный поток и не переносился сюда)."""
        container = QWidget()
...
class NodeGraphCanvas(QWidget):
    ...
    def _on_theme_changed(self, _theme_id: str):
        """Ноды/связи/фон рисуются вручную через QPainter (а не через QSS).
        Ноды сами перечитывают токены темы в paint(), а вот стрелки-связи
        хранят QPen на момент создания - поэтому проще всего полностью
        перестроить canvas при смене темы."""
        self.gscene.setBackgroundBrush(qcolor(theme_manager.tokens().bg_window))
    ...
    def refresh_node_box(self, row: int):
        """Точечное обновление ОДНОЙ карточки после правки поля - без
        полного _rebuild()/set_node(), чтобы не задеть форму редактора,
        которая может как раз сейчас редактироваться (иначе ввод сбрасывался
        на каждую правку)."""
    ...
    def _rebuild_fallback(self):
        """Safety-net: если основная отрисовка упала на каких-то
        нестандартных данных (например, нераспознанные блоки из
        импортированного .rpy), показываем ноды простой колонкой без
        стрелок/групп - лучше так, чем пустой сломанный канвас."""
        self.gscene.clear()
        self._row_items = {}
        self._arrow_items = []
    ...
    def _redraw_arrows(self):
        """Перерисовывает только связи (стрелки), не трогая ноды - используется
        и при полном rebuild, и при live-обновлении во время перетаскивания ноды."""
    ...
    def _on_node_moved(self, row: int, pos: QPointF):
        """Нода перетащена пользователем вручную - запоминаем позицию в модели
        и перерисовываем только стрелки, не трогая остальные ноды."""
        if not self.scene or not (0 <= row < len(self.scene.nodes)):
            return
    ...
    def _start_connecting(self, row: int, port_idx: int, scene_pos: QPointF):
        """Начало перетаскивания стрелки с выходного порта JUMP/MENU-ноды."""
    ...
    def _show_add_node_menu(self):
        """Меню создания новой ноды из тулбара - вставляет после текущей
        выбранной ноды (как и обычная кнопка в списке)."""
    ...
    def align_nodes(self):
        """Кнопка 'Выровнять ноды': пересчитывает древовидную раскладку от
        первого label (или начала сцены), спускаясь по jump/menu-связям,
        и сбрасывает все ручные позиции нод."""
        if not self.scene or not self.scene.nodes:
            return
    ...
    def _auto_layout_tree(self):
        """Раскладывает ноды сцены в виде дерева (обёртка с safety-net: если
        основной алгоритм упадёт на каких-то нестандартных данных, например
        из импортированного .rpy, откатываемся на простую раскладку в одну
        колонку - лишь бы канвас не оставался пустым/сломанным)."""
        try:
    ...
    def _assign_columns_iterative(segments, children):
        """То же самое, что рекурсивный post-order DFS с усреднением колонки
        по детям, но итеративно (явный стек) - защита от RecursionError на
        очень длинных цепочках label/jump (частый случай в импортированных
        больших .rpy-сценариях). Обратные рёбра (циклы, напр. 'вернуться в
        меню') корректно игнорируются, а не превращают все ветки в одну
        колонку."""
        column = {}
        counter = [0]
        for seg in segments:
    ...
    def _auto_layout_tree_impl(self):
        """Раскладывает ноды сцены в виде дерева: каждый label - отдельная
        вертикальная 'дорожка' (колонка), а jump/menu создают ветвление по
        горизонтали к колонке target label - как на референсе."""
        nodes = self.scene.nodes if self.scene else []
        if not nodes:
            return
    ...
    def _delete_selection(self):
        """Del/Backspace в графовом режиме: если выделены стрелки-связи -
        разрываем их (очищаем jump_target/menu choice), если выделены
        ноды - удаляем их целиком. И то и другое можно выделить разом."""
        if not self.scene:
            return
...
class GraphWindow(QWidget):
    """Отдельное окно графового режима, ОТКРЫТОЕ МАКСИМИЗИРОВАННЫМ
    (showMaximized), а не через настоящий OS-level fullscreen
    (showFullScreen). Причина: похоже, именно полноэкранный режим Windows
    ('exclusive'/'borderless' fullscreen, с которым OS может пытаться
    что-то сделать вроде game-mode оптимизаций) - вероятная причина
    молчаливого нативного краша через несколько секунд после открытия,
    воспроизводившегося даже с пустой сценой и без единого клика.
    Maximized-режим визуально почти неотличим, но не убирает системную
    рамку окна - значит остаются штатные кнопки свернуть/развернуть/закрыть
    Windows, которых не было в чистом fullscreen.

    Здесь только canvas - никакой другой виджет приложения (в частности
    NodeEditor) сюда не репарентится, поэтому основное окно вообще не
    трогается и не может 'потечь'."""
    closed = pyqtSignal()
    ...
    def closeEvent(self, e):
        """Отвязываем canvas от этого окна ДО того, как оно начнёт
        уничтожаться, иначе deleteLater() заберёт его с собой."""
