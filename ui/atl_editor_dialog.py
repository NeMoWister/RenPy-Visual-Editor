...
class _AtlPreviewWidget (QWidget ):
    ...
    def paintEvent (self ,event ):
        ...
        if pm is not None and not pm .isNull ():
            ...
            w =self .width ()*max (0.15 ,min (2.5 ,vis ["zoom"]))if self ._is_bg else 90 *max (0.15 ,min (3.0 ,vis ["zoom"]))
            h =self .height ()*max (0.15 ,min (2.5 ,vis ["zoom"]))if self ._is_bg else 150 *max (0.15 ,min (3.0 ,vis ["zoom"]))
            cx =vis ["xalign"]*self .width ()
            cy =vis ["yalign"]*self .height ()
            rect =QRectF (cx -w /2 ,cy -h /2 ,w ,h )if self ._is_bg else QRectF (cx -w /2 ,cy -h ,w ,h )
            p .setOpacity (max (0.0 ,min (1.0 ,vis ["alpha"])))
