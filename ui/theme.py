...
Архитектура:
    * ThemeTokens - набор именованных цветов/градиентов одной темы.
    * THEMES      - реестр всех доступных тем (id -> ThemeTokens).
    * ThemeManager - применяет тему к QApplication (палитра + QSS),
      хранит текущую тему и умеет плавно проигрывать переход (fade).

Как добавить новую тему:
    1. Скопировать любой блок `register_theme(ThemeTokens(...))` ниже.
    2. Поменять id/display_name и цвета.
    3. Тема автоматически появится в списке (theme_manager.available())
       и в выпадающем списке в "Настройках редактора" - ничего больше
       менять не нужно.

Все темы в этом наборе - тёмные (юзабельные тёмные цветовые схемы),
но токенов достаточно, чтобы при желании добавить и светлую тему.

Если установлен пакет PyQt6-Fluent-Widgets (qfluentwidgets), при смене
темы синхронизируется его акцентный цвет/режим, чтобы виджеты
QFluentWidgets, используемые в приложении, визуально совпадали с
...
def register_theme(tokens: ThemeTokens) -> None:
    """Регистрирует тему в общем реестре (используется и для встроенных,
    и для любых пользовательских тем)."""
...
def qcolor(value: str) -> QColor:
    """Превращает цвет темы (hex '#rrggbb' ИЛИ css-строка вида
    'rgba(r, g, b, a)') в QColor - для использования в ручной QPainter-
    отрисовке (canvas нод, GlassPanel и т.п.), где токены темы нужны не
    только как QSS, но и как реальные QColor."""
    value = value.strip()
...
def _cyberpunk_css(accent_1: str, accent_2: str, accent_1_dark: str, btn_text: str) -> str:
    """Общий генератор 'фактуры' для киберпанк-тем: резкие углы, плоские
    HUD-кнопки с акцентной рамкой вместо мягкого стекла, ЗАГЛАВНЫЕ буквы
    на кнопках."""
    return f"""
...
def fade_in_widget(widget: QWidget, duration: int = 260, start: float = 0.0, end: float = 1.0) -> QPropertyAnimation:
    """Плавное появление виджета (окна/диалога/панели). Ссылка на анимацию
    сохраняется на самом виджете, чтобы её не собрал сборщик мусора."""
...
def crossfade_theme_switch(widget: QWidget, duration: int = 180) -> None:
    """Короткое затемнение/высветление окна в момент смены темы, чтобы
    переход между палитрами не выглядел резким миганием."""
...
class ThemeManager(QObject):
    """Хранит текущую тему и применяет её ко всему приложению."""
...
def apply_dark_theme(app: QApplication, theme_id: str = DEFAULT_THEME_ID) -> None:
    """Точка входа для обратной совместимости: применяет тему (по
    умолчанию - тему, ранее единственную в проекте, теперь называется
    "ember")."""
...
def fit_window_to_screen(widget: QWidget, desired_w: int, desired_h: int,
    ...
    """Задаёт размер окна/диалога с оглядкой на РЕАЛЬНО доступное место на
    экране - а не слепо на фиксированные пиксели, как раньше почти везде
    в проекте (setMinimumSize(1500, 940) и т.п.). На ноутбуках с масштабом
    Windows 125%/150% логическое разрешение рабочего стола заметно меньше
    физического (напр. 1920x1080 при 150% - это всего 1280x720 логических
    пикселей для Qt), и такие фиксированные минимумы оказывались больше
    доступного места - окно вылезало за края экрана.

    desired_w/desired_h - предпочтительный размер (как раньше передавали в
    resize()); min_w/min_h - предпочтительный минимум (как в
    setMinimumSize()), по умолчанию равен desired. Оба при необходимости
    ужимаются под screen_fraction (по умолчанию 92%) от доступной области
    экрана - Qt6 сам уже переводит физические пиксели в логические с учётом
    масштаба ОС (см. HighDpiScaleFactorRoundingPolicy в main.py), так что
    здесь остаётся только не запрашивать больше, чем реально есть."""
    if min_w is None:
        min_w = desired_w
    if min_h is None:
        min_h = desired_h
