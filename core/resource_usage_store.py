"""
Избранные и недавно использованные ресурсы (фоны, CG, спрайты, музыка,
звуки, эмбиенс) - для карусели ресурсов (ui/resource_carousel.py).

Хранится глобально в базовой папке приложения (как теги и переопределения
имён ресурсов), не привязано к конкретному проекту - секция "resource_usage"
общего editor_config.json.

Формат:
{
    "enabled": true,                          # показывать ли блок вверху карусели
    "favorites": {"bg": ["beach", "forest"]},  # избранное по категориям
    "recent": {"bg": ["forest", "beach"]},     # недавние по категориям (сначала новые)
}
"""
...
class ResourceUsageStore:
    ...
    def toggle_favorite(self, category: str, var_name: str) -> bool:
        """Переключает избранное, возвращает новое состояние (True/False)."""
