"""
Экспорт проекта в НЕСКОЛЬКО .rpy файлов (например, по одному на главу/акт)
вместо одного большого сценария. Переиспользует ту же построчную генерацию
нод, что и core.code_generator.generate_full_script - просто применяет её к
подмножествам сцен и оборачивает каждое подмножество в самостоятельный,
валидный сам по себе .rpy-файл.

Правила разбиения (SplitRule):
  - "scene"  - один файл на каждую сцену редактора.
  - "label"  - один файл на каждый "верхнеуровневый" label (группирует все
               сцены от одной LABEL-ноды до следующей - ближе всего к
               привычному делению на главы/акты в самом Ren'Py).
  - "count"  - фиксированное число сцен на файл (простое ручное разбиение).

Все файлы вместе, загруженные Ren'Py, работают как один сценарий: jump/call
между главами работают как обычно, т.к. Ren'Py сам объединяет все .rpy файлы
проекта в одно пространство имён меток при компиляции.
"""
import re
...
def split_project(project: Project, rule: str, rm=None,
    ...
    """Возвращает список ExportChunk - по одному на выходной файл, в порядке
    следования. Каждый файл - самостоятельный валидный .rpy: если группа
    сцен не начинается с настоящей LABEL-ноды, для файла заводится
