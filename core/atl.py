...
"""
Парсер и "честный" эмулятор Ren'Py ATL (Animation & Transformation Language) -
блоков вида:
    ...
    show et_mon_1 with dissolve:
        "et_mon_1" with dissolve
        pause 1.0
        "et_mon_2" with dissolve
        pause 1.0
        repeat

    scene bg et_int_liaz with flash:
        anchor (0.0, 0.0) pos (0.0, 0.0)
        linear 0.1 pos (-9, -7)
        linear 0.1 pos (0, 0)
        repeat

Этот модуль НЕ используется для генерации кода (текст ATL-блока хранится
дословно в SceneNode.atl_script и переносится в .rpy как есть - см.
code_generator.py / rpy_script_import.py) - он нужен только для того, чтобы
честно ПРОИГРАТЬ блок во времени в предпросмотре/презентации: вычислить,
какие свойства спрайта/фона (позиция, зум, поворот, прозрачность) и какая
картинка ("image"-статement, "et_mon_2" и т.п.) должны быть показаны в
произвольный момент t секунд после появления ноды, с учётом linear/ease-
интерполяции, pause и repeat (с числом повторов или бесконечного).

Всё, что не распознано (условные конструкции, произвольные python-вызовы,
event/function/on/contains и т.п.) сохраняется как ATLRaw - не ломает разбор
остального блока, просто не анимируется (мгновенный no-op в таймлайне).
"""
import re
...
def parse_atl_text(text: str) -> ATLBlock:
    """Разбирает ДЕДЕНТНУТЫЙ (relative-indent) текст ATL-блока (как хранится
    в SceneNode.atl_script) в дерево statement-ов."""
...
def evaluate(tl: CompiledTimeline, t: float) -> Dict[str, object]:
    """Возвращает состояние свойств на момент t секунд от начала таймлайна."""
    if not tl.segments:
...
def resolve_visual(atl_text: str, t: float,
    ...
    """Возвращает "честно" вычисленное на момент t состояние трансформации:
    {xalign, yalign, zoom, alpha, rotate, image_text, image_transition}.
    image_text - если ATL-блок переключает картинку строкой-статементом
    (напр. "et_mon_2" внутри show ... with dissolve: ...) - имя, которое
    нужно показать ВМЕСТО базового изображения ноды; None - показывать
    базовое изображение как обычно."""
    base_state = {
...
def quick_final_position(atl_text: str, base_xalign: float = 0.5,
    ...
    """Финальное (последний кадр без учёта repeat) xalign/yalign/zoom -
    используется как приближение для полей, которые не умеют анимацию
    (комбобокс якоря и т.п.)."""
    if not atl_text or not atl_text.strip():
...
def cycle_duration(atl_text: str) -> Optional[float]:
    """Длительность одного цикла таймлайна в секундах.
    None означает "не зациклено" (repeat отсутствует) ИЛИ "бесконечный
    repeat без числа" - в обоих случаях вызывающая сторона просто гоняет
    свой собственный QTimer и оборачивает t сама по необходимости; для
    конечного repeat N возвращает полную длительность N проходов."""
    if not atl_text or not atl_text.strip():
        return None
    try:
        block = parse_atl_text(atl_text)
...
def is_animated(atl_text: str) -> bool:
    """Есть ли в блоке хоть что-то, что реально меняется во времени
    (интерполяция, смена картинки, повтор) - используется UI, чтобы решить,
    запускать ли таймер живого предпросмотра."""
    if not atl_text or not atl_text.strip():
        return False
    try:
        block = parse_atl_text(atl_text)
    ...
    if tl.has_loop:
        return True
    return any(s.kind == 'interp' for s in tl.segments) or \
        any(s.state_to.get('__image__') is not None for s in tl.segments)


def referenced_images(atl_text: str) -> List[str]:
    """Список имён картинок, на которые переключается ATL-блок через
    строковые image-статементы ("et_mon_2" with dissolve и т.п.) - в порядке
    первого появления, без дублей. Используется UI, чтобы заранее разрешить
    и подгрузить все варианты картинки для честного покадрового предпросмотра
    (см. ui/scene_preview.py / ui/presentation_window.py)."""
    names: List[str] = []
    seen = set()
...
def describe(atl_text: str) -> List[str]:
    """Список человекочитаемых предупреждений о нераспознанных строках
    блока (для UI редактора ATL) - не блокирует сохранение, просто
    информирует, что эти строки не будут анимированы в предпросмотре (но
    попадут в итоговый .rpy дословно)."""
    warnings: List[str] = []
