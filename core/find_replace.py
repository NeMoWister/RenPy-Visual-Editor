"""
Массовая замена текста по всем репликам сценария (find & replace), в
отличие от замены, доступной только при импорте .rpy.
...
которая мутирует project и возвращает количество произведённых замен.
"""
import re
...
def _iter_text_fields(node: SceneNode, include_comments: bool):
    """Возвращает список (label, get_text) для текстовых полей ноды,
    которые имеет смысл искать/заменять."""
    fields = []
...
def apply_replace_all(project: Project, query: str, replacement: str, case_sensitive: bool = False,
    ...
    """Заменяет ВСЕ вхождения query на replacement по всему проекту.
    Возвращает суммарное количество произведённых замен (по вхождениям,
    не по нодам)."""
    if not project or not query:
        return 0
    pattern = _pattern(query, case_sensitive, whole_word)
